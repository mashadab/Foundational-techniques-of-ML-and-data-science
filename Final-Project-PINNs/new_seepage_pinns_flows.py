...
class SeepagePINNFlowsNew:
    ...
    def net_f(self, x, q):
        ...
        f = q + kappa * u * u_x + q / 3.0 * u_x * u_x + q / 3.0 * u * u_xx
...
class SeepageAnalyticNucci:
    ...
    def eval(self, xx, Q):
        return np.sqrt(self.h1**2 - 2 * Q * (self.L - xx)/(self.W*self.k) + 2.0/3.0 * Q**2.0/(self.W*self.k)**2.0 * (1.0 - np.exp(-3* (self.W*self.k)*(self.L - xx)/Q)))
