...
class PhysicsInformedNN:
    ...
    def __init__(self, X, X_left_boundary, X_right_boundary, u, a, b, layers, lb, ub, alpha=1, beta=1, optimizer_type="adam"):
        self.lb = lb
        self.ub = ub
        ...
        self.beta = beta 
        self.beta_const = tf.constant(self.beta, dtype=tf.float32, shape=[1,1])
        ...
        self.a_const = tf.constant(self.a, dtype=tf.float32, shape=[1,1])
        ...
        self.lambda_1 = tf.Variable([np.log(0.05)], dtype=tf.float32)
        ...
        self.loss = tf.reduce_mean(tf.square(self.u_tf - self.u_pred)) \
                    ...
                    + self.beta_const * tf.reduce_mean(tf.square(self.f_left_boundary)) \
                    + self.beta_const * tf.reduce_mean(tf.square(self.f_right_boundary)) 
...
def plot_timestamps(x, dt, FD_sol, model, ind_tests, FD_fit=None):
    ...
    for ind in ind_tests:
        ...
        plt.savefig("unsteady_figures/unsteady_k_res_%d.png" %(ind))
        plt.close()
...
if __name__ == "__main__":
    ...
    if load_model:
        ...
        model = PhysicsInformedNN(X_u_train, X_left_boundary, X_right_boundary, 
                u_train, a, b, layers, lb, ub, alpha=alpha, beta=beta, optimizer_type="adam") 
